  ...
  async signWithFreighter(challenge: Vend402Challenge): Promise<string> {
    ...
    try {
      ...
      const txn = new TransactionBuilder(sourceAccount, {
        fee: BASE_FEE,
        networkPassphrase: this.network === "stellar-testnet" ? Networks.TESTNET_NETWORK_PASSPHRASE : Networks.PUBLIC_NETWORK_PASSPHRASE,
      })
        .addOperation(
          Operation.payment({
            destination: challenge.destination,
        ...
        .setDefaultMaxOpCount()
        .build()

      // Request Freighter to sign
      const signed = await window.freighter.signTransaction(txn.toXDR(), {
        networkPassphrase: this.network === "stellar-testnet" ? Networks.TESTNET_NETWORK_PASSPHRASE : Networks.PUBLIC_NETWORK_PASSPHRASE,
      })
