...
// Environment
const MERCHANT_ACCOUNT = Deno.env.get("VEND402_MERCHANT_ACCOUNT") || "GBZBER5IXM4TSI2WFRNNBFO4W7C74CNOSHUT57WOSUTJTJOZXZKVR335"
...
async function handleGetChallenge(deviceId: string): Promise<Response> {
  try {
    ...
    if (storeError) {
      ...
      // Debugging: Fail the request so we can see why
      return jsonResponse({ error: "DB Error: " + JSON.stringify(storeError), keyPresent: !!SUPABASE_SERVICE_KEY }, 500)
    }
...
async function handleVerifyPayment(deviceId: string, txHash: string, challengeId?: string): Promise<Response> {
  try {
    ...
    // Fetch operations separately (Horizon /transactions/{hash} doesn't include them)
    const opsResponse = await fetch(`${HORIZON_URL}/transactions/${txHash}/operations`)
    if (!opsResponse.ok) {
      throw new Error(`Failed to fetch operations: ${opsResponse.statusText}`)
    }
    ...
    if (insertError) {
      ...
      // Debugging: Fail request on error
      throw new Error("DB Error: " + JSON.stringify(insertError))
    }
